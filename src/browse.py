...
def get_repos(api_key, timeout=0):
    ...
    If the cache is empty, use the embedded Github library
    ...
    return wf.cached_data(
        REPO_CACHE, lambda: fetch_repos(API_ROOT, api_key), max_age=timeout)


def main(wf):
    """Main entrypoint for workflow."""
    ...
    if args['unset']:
        try:
            ...
            notify(
                "Removed API Token from Keychain",
                "Add a new token in order to use the workflow.")
    ...
    if args['timeout']:
        try:
            ...
            raise Exception(
                "Got non-numeric input '{}'".format(args['TIMEOUT']))
        return 0
    ...
    try:
        ...
        raise Exception(
            "Missing Github token, set using 'githubsettings' keyword.")

    if args['refresh']:
        wf.clear_cache(lambda name: name == "{}.cpickle".format(REPO_CACHE))
        notify(
            "Reindexing Github Repos",
            "I'll let you know once I've cleaned things up.")
        get_repos(api_key)
...
if __name__ == '__main__':
    wf = Workflow3(
        update_settings={
            "github_slug": "dishbreak/alfred3-github-repo-browser"
        }
    )
